...
export default function ExamPage() {
  ...
  const handleFinish = async () => {
    try {
      // Format answers as expected by the API: { [questionId]: [optionId] }
      const answers = Object.entries(selectedOptions).reduce((acc, [qId, optId]) => {
        acc[qId] = [optId];
        return acc;
      }, {} as Record<string, string[]>);

      const res = await fetch(`/api/candidate/exams/${params.id}/submit`, {
        method: 'POST',
